 ...
 */
package com.accreditation.app.controller;

import com.estacionesserver.model.History;
import com.estacionesserver.model.User;
import com.estacionesserver.repository.HistoryRepository;
import com.estacionesserver.repository.UserRepository;
...
//// </editor-fold>
    ...
    @RolesAllowed({"admin"})
    @Path("email")
    @Operation(summary = "Busca un user por el usernam", description = "Busqueda de user por email")
    @APIResponse(responseCode = "200", description = "El user")
    @APIResponse(responseCode = "404", description = "Cuando no existe el username")
    @APIResponse(responseCode = "500", description = "Servidor inalcanzable")
    @Tag(name = "BETA", description = "Esta api esta en desarrollo")
    ...
    public User findByEmail(@QueryParam("email") final String email) {
        ...
        return userRepository.findByEmail(email).orElseThrow(
                () -> new WebApplicationException("No hay user con ese email " + email, Response.Status.NOT_FOUND));

    }
//// </editor-fold>  
      // <editor-fold defaultstate="collapsed" desc="User findByCedula(@QueryParam("cedula") final String cedula)">

    @GET
    @RolesAllowed({"admin"})
    @Path("identificationcard")
    @Operation(summary = "Busca un user por el usernam", description = "Busqueda de user por cedula")
    @APIResponse(responseCode = "200", description = "El user")
    @APIResponse(responseCode = "404", description = "Cuando no existe el username")
    @APIResponse(responseCode = "500", description = "Servidor inalcanzable")
    @Tag(name = "BETA", description = "Esta api esta en desarrollo")
    ...
    public User findByIdentificationcard(@QueryParam("identificationcard") final String identificationcard) {
        System.out.println("\t findByIdentificationcard: "+identificationcard);
        return userRepository.findByIdentificationcard(identificationcard).orElseThrow(
                () -> new WebApplicationException("No hay user con esa cedula " + identificationcard, Response.Status.NOT_FOUND));
